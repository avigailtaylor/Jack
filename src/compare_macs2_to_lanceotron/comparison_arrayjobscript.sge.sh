...
SAMPLE=`cat meta/samples.txt | tail -n+${SGE_TASK_ID} | head -1 | cut -f1`

rm -rf job_outputs/comparison_output/${SAMPLE}.macs2_vs_lanceotron_peaks.txt
touch job_outputs/comparison_output/${SAMPLE}.macs2_vs_lanceotron_peaks.txt
rm -rf temp/${SAMPLE}_1
mkdir temp/${SAMPLE}_1

MACS2_N_FILE_RAW="../macs2/job_outputs/macs2_output/${SAMPLE}_Inputsample_peaks.narrowPeak"
MACS2_B_FILE_RAW="../macs2broad/job_outputs/macs2broad_output/${SAMPLE}_Inputsample_peaks.broadPeak"
LANCE_FILE_RAW="../lanceotron/job_outputs/lanceotron_output/Inputsample_${SAMPLE}_L-tron.bed "

MACS2_N_FILE="temp/${SAMPLE}_1/${SAMPLE}.narrowPeak.simple.bed"
MACS2_B_FILE="temp/${SAMPLE}_1/${SAMPLE}.broadPeak.simple.bed"
LANCE_FILE="temp/${SAMPLE}_1/${SAMPLE}.lanceotron.simple.bed"

cut -f1-3 ${MACS2_N_FILE_RAW} > ${MACS2_N_FILE}
cut -f1-3 ${MACS2_B_FILE_RAW} > ${MACS2_B_FILE}
tail -n+2 ${LANCE_FILE_RAW} | awk '{if($4>0.8) print $0}' | cut -f1-3 > ${LANCE_FILE}

module load BEDTools/2.30.0-GCC-10.2.0

echo "Comparing macs2 and lanceotron peak calls in ${SAMPLE}" > job_outputs/comparison_output/${SAMPLE}.macs2_vs_lanceotron_peaks.txt
echo "" >> job_outputs/comparison_output/${SAMPLE}.macs2_vs_lanceotron_peaks.txt

MACS2_N_CALLS=`cat ${MACS2_N_FILE} | wc -l`
MACS2_B_CALLS=`cat ${MACS2_B_FILE} | wc -l`
LANCE_CALLS=`cat ${LANCE_FILE} | wc -l`

echo "macs2 narrow calls: ${MACS2_N_CALLS}" >> job_outputs/comparison_output/${SAMPLE}.macs2_vs_lanceotron_peaks.txt
echo "macs2 broad calls: ${MACS2_B_CALLS}" >> job_outputs/comparison_output/${SAMPLE}.macs2_vs_lanceotron_peaks.txt
echo "lanceotron calls: ${LANCE_CALLS}" >> job_outputs/comparison_output/${SAMPLE}.macs2_vs_lanceotron_peaks.txt
...
awk -F' ' '{printf "%s %.2f %.2f %.2f %.2f\n" , $1, 100*($2/$6), 100*($3/$6), 100*($4/$6), 100*($5/$6)}'  >> job_outputs/comparison_output/${SAMPLE}.macs2_vs_lanceotron_peaks.txt 
